...
def prettyfy(error, fileTest, file):
    ...
    try:
        ...
        error = sg.popup(
            "Error 424: Something went wrong with writing to the file", icon=ic)
        pass
    if not error:
...
def assignAtr(listKnownIDs, root, link, el):
...
def singleFile(bpmnPath, xmlPath):
    ...
    if bpmnName != xmlName:
        ...
        prettyfy(error, fileTest, file)

    # Save the file to the dirsctory of bpmn
    currentDir = "/".join(bpmnPath.split("/")[:-1])
    try:
        fileTest = open(currentDir + "/" + bpmnName +
    ...
    tree = ET.parse(bpmnPath)
    root = tree.getroot()

    # Get the link from xml files to better search for brackets (<task>)
    treeXML = ET.parse(xmlPath)
    ...
    for activity in rootXML.iter(linkXML + "}ActivitySet"):
        ...
        # Find definition ID
        definitionsId = root.attrib["id"][5:]
        ...
        root.set("xmlns:bwl0",
                 "http://www.ibm.com/WebSphere/bpm/BlueworksLive/Glossaries")
    ...
    for i in root.iter(link + "}definitions"):
        ...
        for j in i.iter(link + "}process"):
            ...
            for r in j.iter():
                ...
                if r.tag == link + "}startEvent":
                    assignAtr(listKnownIDs, root, link, r)
        ...
        if insertSubProcessID == 1:
            listKnownIDs.insert(len(listKnownIDs)-1, subProcessID)
