...
<div class="container grid grid--blog-col ">
  ...
  <article>
    ...
    <?php
    if ($categoriesFound) {
     foreach ($categoriesFound as $categoryFound) {
        echo "<h3>" . $categoryFound['category'] . $categoryFound['id'] . "<br/> </h2>";
        $pagesFound = Page::getPages($categoryFound['id']);
        if($pagesFound) {
          foreach ($pagesFound as $pageFound) {
            // echo $pageFound['id'] . $pageFound['title'] . $pageFound['description'] . '<br/>';
        //     	// echo '<div><h4><a href="page.php?id=' . $row['id'] . '">' . htmlspecialchars($row['title']) . '</a></h4><p>' . htmlspecialchars($row['description']) . '</p></div>';
            echo "<div><h4><a href=\"page.php?id={$pageFound['id']}\">{$pageFound['title']}</a></h4><p>{$pageFound['description']}</p></div>\n";
          }
        }
  ...
  // echo "<div><h4><a href=\"page.php?id={$noteFound['page_id']}\">{$noteFound['note']}</a></h4><p>{$noteFound['note']}</p></div>\n";
     ?>
  </article>
