...
$page_title = 'The Highest Rated Pages';
...
// Validate the category ID:
  $getHighestRatedPages = Page::getHighestRatedPages();


  if (!$getHighestRatedPages) { // Problem
    $page_title = 'Error!';
...
<div class="container grid grid--blog-col ">
  ...
  <article>
    ...
    <?php
    if ($getHighestRatedPages) {
     foreach ($getHighestRatedPages as $getHighestRatedPage) {
        ...
        echo "<div><h4><a href=\"page.php?id={$getHighestRatedPage['id']}\">{$getHighestRatedPage['title']}</a></h4><p>{$getHighestRatedPage['average']}</p></div>\n";
    }
  }
     ?>
  </article>
