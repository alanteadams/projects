...
// Validate the category ID:
  $getNotesFound = Page::getNotes();


  if (!$getNotesFound) { // Problem
    $page_title = 'Error!';
...
<div class="container grid grid--blog-col ">
  ...
  <nav class="main-nav">
    <ul class="main-nav-list">
      ...
      <a class="main-nav-link" href="./logout">Logout</a>
    </ul>
  </nav>
  ...
  <article>
    ...
    <?php
    if ($getNotesFound) {
     foreach ($getNotesFound as $getNoteFound) {
        ...
        echo "<div><h4><a href=\"page.php?id={$getNoteFound['page_id']}\">{$getNoteFound['title']}</a></h4><p>{$getNoteFound['note']}</p></div>\n";
    }
  }
     ?>
  </article>
